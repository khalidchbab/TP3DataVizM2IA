...
function updateViz(json, data, value) {
    d3.select('#day').html(daysArray[value]);
    drawMap(json, data, daysArray[value]);
}

function drawMap(json, data, currentWeek) {
    ...
    map.enter()
        .append("path")
        .attr('id', d => "dep-" + d.properties.nom)
        .attr("d", path)
...
d3.csv("./TP3DataVizM2IA/data/covid-france-mars-avril.csv").then(function (data) {
    ...
    d3.json("./TP3DataVizM2IA/data/map-france.geojson").then(function (json) {
        ...
        d3.select("#slider").on("input", function () {
            ...
            updateViz(json, data, this.value);
        });
        drawMap(json, data, daysArray[0])
